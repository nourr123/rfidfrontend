...
<<<<<<< HEAD
  ...
  Future<Attendance?> getPointageJour(String uid, String date) async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/pointage/$uid/$date'),
        headers: _headers,
      );

      if (response.statusCode == 200) {
        return Attendance.fromApi(json.decode(response.body));
      }
      return null;
    } catch (e) {
      return null;
  ...
  // Vérifier si l'utilisateur est actuellement pointé (présent)
  Future<bool> isUserPresent(String uid) async {
    try {
      final today = _getTodayKey();
      final pointage = await getPointageJour(uid, today);

      if (pointage == null) return false;

      // Vérifier s'il y a une session active (entrée sans sortie)
      for (var session in pointage.sessions) {
        if (session.entree != null && session.sortie == null) {
          return true;
        }
      }
      return false;
  ...
  // Obtenir le temps total travaillé aujourd'hui
  Future<Duration?> getTravailAujourdhui(String uid) async {
    try {
      final today = _getTodayKey();
      final pointage = await getPointageJour(uid, today);
      ...
      for (var session in pointage.sessions) {
        if (session.entree != null && session.sortie != null) {
          final entre = _parseTime(session.entree!);
          final sortie = _parseTime(session.sortie!);
          total += sortie.difference(entre);
        }
      }
      return total;
    } catch (e) {
  ...
  // Helper pour obtenir la date du jour au format dd-MM-yyyy
  String _getTodayKey() {
  ...
  // Helper pour parser une heure HH:MM:SS
  ...
  // Simuler un pointage RFID (pour test)
  // Cette méthode écrit directement dans Firebase via le backend
  Future<bool> simulerPointage(String uid, String rfidTag) async {
    try {
      // Appeler une route spéciale pour le pointage
      final response = await http.post(
        Uri.parse('$baseUrl/pointage/simuler'),
        headers: _headers,
        body: json.encode({
          'uid': uid,
          'rfidTag': rfidTag,
